...
import DeviceModuleMenu from "./menu-items/Devices";
...
import Ticketv3ModuleMenu from "./menu-items/Ticketv3";
import Ticketv1ModuleMenu from "./menu-items/Ticketv1";
...
const ModuleMenu = ({ modules, dict, build }: Props) => {
  ...
  console.log('Modules:', JSON.stringify(modules)); // Log the modules array
  console.log('Dict:', JSON.stringify(dict.ModuleMenu)); // Log the dict object
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <div className="pt-6">
          ...
          {modules.find(
            (menuItem: any) => menuItem.name === "ticketv1" && menuItem.enabled
          ) ? (
            <Ticketv1ModuleMenu open={open} title={dict.ModuleMenu.ticketv1} />
          ) : null}
          {modules.find(
            (menuItem: any) => menuItem.name === "ticketv3" && menuItem.enabled
          ) ? (
            <Ticketv3ModuleMenu open={open} title={dict.ModuleMenu.ticketv3} />
          ) : null}
          {modules.find(
            (menuItem: any) => menuItem.name === "devices" && menuItem.enabled
          ) ? (
            <DeviceModuleMenu open={open} title={dict.ModuleMenu.devices} />
          ) : null}
          {modules.find(
